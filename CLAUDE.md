...
`emdx delegate` is the **one command for all one-shot AI execution**. It replaces the
deprecated `agent`, `run`, and `each` commands. Results print to stdout (so Claude
reads them inline) AND persist to emdx.
...
**Note:** `--chain` and `--synthesize` are mutually exclusive.

## ⚠️ Deprecated Commands

The following commands still work but show deprecation warnings. Use `emdx delegate` instead:

| Deprecated | Replacement |
|------------|-------------|
| `emdx agent "task" --pr` | `emdx delegate "task" --pr` |
| `emdx run "t1" "t2"` | `emdx delegate "t1" "t2"` |
| `emdx run --worktree "fix X"` | `emdx delegate --worktree "fix X"` |
| `emdx each --from "cmd" --do "task"` | `emdx delegate "t1" "t2"` (pre-expand tasks) |
