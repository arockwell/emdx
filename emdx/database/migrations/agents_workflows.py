"""Agent and workflow system migrations (migrations 007-015).

These migrations establish:
- Agent system (later removed in migration 024)
...
def migration_015_add_export_profiles(conn: sqlite3.Connection):
    ...
    # Create indexes
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_export_profiles_name ON export_profiles(name)"
    )
    cursor.execute(
