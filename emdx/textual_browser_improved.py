#!/usr/bin/env python3
"""
Improved modal browser for emdx using Textual's MarkdownViewer widget.

This provides:
- Better markdown rendering with MarkdownViewer
- Code syntax highlighting with configurable themes
...
from textual.widgets import Header, Footer, DataTable, Input, Label, Static, MarkdownViewer
...
class DocumentBrowser(App):
    """Modal document browser with vim-style navigation and improved markdown rendering."""
    ...
    def __init__(self):
        ...
        # Use 'manni' theme for light terminals, 'monokai' for dark
        self.code_theme = "monokai"  # You can make this configurable
        ...
        with Horizontal():
            ...
            with Container(id="preview"):
                yield MarkdownViewer(
                    id="preview-content", 
                    show_table_of_contents=False,  # Don't need TOC for preview
                    code_dark_theme=self.code_theme,
                    code_light_theme="manni"  # Good for light terminals
                )
    ...
    def update_preview(self, doc_id: int):
        ...
        try:
            ...
            if doc:
                markdown_viewer = self.query_one("#preview-content", MarkdownViewer)
...
{doc['content']}"""
                ...
                markdown_viewer.document.update(markdown_content)
                # Scroll to top
                markdown_viewer.scroll_home()
        except Exception as e:
            markdown_viewer = self.query_one("#preview-content", MarkdownViewer)
            markdown_viewer.document.update(f"# Error\n\nError loading preview: {e}")
    ...
    def action_edit(self):
        ...
        if self.current_doc_id and self.mode == "NORMAL":
            self.exit()
    ...
    def action_delete(self):
        ...
        if self.current_doc_id and self.mode == "NORMAL":
            # For now, just exit and run delete
    ...
    def action_view(self):
        ...
        if self.current_doc_id and self.mode == "NORMAL":
            ...
            if result.returncode == 0:
                cmd = [sys.executable, '-m', 'emdx.cli', 'view', str(self.current_doc_id), 
                ...
                subprocess.run([sys.executable, '-m', 'emdx.cli', 'view', str(self.current_doc_id)])
