"""Claude execution service - core execution logic.

This module contains the core Claude execution functions extracted from
commands/claude_execute.py to break bidirectional dependencies between
the commands and services layers.

The CLI commands remain in commands/claude_execute.py and import from here.
"""
            ...
            for item in content:
                if item.get("type") == "tool_result":
                    ...
                    if len(str(item.get("content", ""))) > 100:
                        result_content += "..."
                    return f"{format_timestamp(timestamp)} 📤 Tool result: {result_content}"

        elif data.get("type") == "result":
            # Final result
            cost = data.get("cost_usd", 0)
            duration = data.get("duration_ms", 0) / 1000
            return f"{format_timestamp(timestamp)} 🏁 Completed (${cost:.4f}, {duration:.1f}s)"

    except json.JSONDecodeError:
        # Not JSON, return as plain text with timestamp
        if line.strip():
