...
class ParallelStrategy(ExecutionStrategy):
    ...
    async def execute(
        ...
        for i in range(num_runs):
            ...
            individual_runs.append((individual_run_id, prompt, i))
        ...
        async def run_with_worktree(run_id: int, prompt: str, run_number: int, run_index: int):
            ...
            try:
                async with pool.acquire(target_branch=f"parallel-{stage_run_id}-{run_number}") as worktree:
                    ...
                    result = await run_agent(
                        ...
                        context=run_context,
                        title=make_title(stage, run_index, run_id),
                    )
                    completed_count += 1
        ...
        try:
            tasks = [
                run_with_worktree(run_id, prompt, i + 1, run_index)
                for i, (run_id, prompt, run_index) in enumerate(individual_runs)
            ]
