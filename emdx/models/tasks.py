...
def get_recent_failures(hours: int = 24, limit: int = 5) -> list[TaskDict]:
    """Get recent failed top-level delegate tasks for prime context.

    Returns failed tasks from the last N hours with prompt field populated
    (delegate tasks only, not manually created tasks).
    """
    with db.get_connection() as conn:
        cursor = conn.execute(
            """
