...
class VimEditTextArea(TextArea):
    ...
    def _update_line_numbers(self):
        ...
        try:
            ...
            if not hasattr(self, 'text') or self.text is None:
                key_logger.info("⚠️ STEP 3: text is None, defaulting to empty")
                total_lines = 1
            else:
            ...
            if current_line < 0:
                key_logger.info(f"⚠️ STEP 3: current_line {current_line} < 0, clamping to 0")
                current_line = 0
            elif current_line >= total_lines:
                key_logger.info(
                    f"⚠️ STEP 3: current_line {current_line} >= total_lines {total_lines}, "
    ...
    def _handle_command_mode(self, event: events.Key) -> None:
        ...
        if event.key == "escape":
            ...
            hasattr(event, 'character') and event.character and
            hasattr(event, 'is_printable') and event.is_printable
        ):
