...
class SaveFileModal(ModalScreen[Optional[dict]]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="save-modal-container"):
            ...
            with Vertical(id="modal-content"):
                ...
                yield Input(placeholder="e.g., notes, python, refactor", id="tags-input")
    ...
    def action_save(self) -> None:
        ...
        try:
            ...
            # Return success
            self.dismiss({"success": True, "doc_id": doc_id, "title": title, "tags": unique_tags})
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="execute-modal-container"):
            ...
            with Vertical(id="modal-content"):
                ...
                if self.doc_id:
                    yield Static(
                        f"✅ File is saved in EMDX (#{self.doc_id})", classes="success-text"
                    )
                else:
                    yield Static(
                        "⚠️ File not saved to EMDX. Save first for better tracking.",
    ...
    def action_execute(self) -> None:
        ...
        if self.doc_id:
            ...
            try:
                ...
                self.dismiss(
                    {"success": True, "execution_id": execution_id, "log_file": str(log_file)}
                )
            except Exception as e:
                self.dismiss({"success": False, "error": str(e)})
        else:
            # For unsaved files, we need to save first
            self.dismiss({"success": False, "error": "Please save the file to EMDX first"})
