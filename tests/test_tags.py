...
class TestTagOperations:
    ...
    def test_add_tags_to_document(self):
        ...
        conn.execute(
            "INSERT INTO document_tags (document_id, tag_id) VALUES (?, ?)", (doc_id, python_tag_id)
        )
        conn.execute(
    ...
    def test_search_by_tags(self):
        ...
        # Tag documents
        conn.execute(
            "INSERT INTO document_tags (document_id, tag_id) VALUES (?, ?)", (doc1, python_id)
        )
        conn.execute(
            "INSERT INTO document_tags (document_id, tag_id) VALUES (?, ?)", (doc2, python_id)
        )
        conn.execute(
            "INSERT INTO document_tags (document_id, tag_id) VALUES (?, ?)", (doc2, testing_id)
        )
    ...
    def test_remove_tag_from_document(self):
        ...
        conn.execute(
            "INSERT INTO document_tags (document_id, tag_id) VALUES (?, ?)", (doc_id, keep_id)
        )
        conn.commit()
    ...
    def test_list_all_tags(self):
        ...
        tags_data = [("python", 5), ("javascript", 3), ("testing", 8), ("docker", 1)]

        for name, usage in tags_data:
            conn.execute("INSERT INTO tags (name, usage_count) VALUES (?, ?)", (name, usage))
        conn.commit()
