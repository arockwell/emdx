...
class TestBackwardCompatibility:
    """Test that documents.py functions still work and dual-write."""

    def test_documents_update_stage_dual_writes(self, setup_test_db):
        """Test that update_document_stage writes to both tables."""
    ...
    def test_documents_update_pr_url_dual_writes(self, setup_test_db):
        """Test that update_document_pr_url writes to both tables."""
        from emdx.database.documents import save_document, update_document_stage, update_document_pr_url
        from emdx.database import cascade as cascade_db
    ...
    def test_documents_get_cascade_stats_reads_new_table(self, setup_test_db):
        """Test that get_cascade_stats reads from new table."""
        from emdx.database.documents import get_cascade_stats, save_document
        from emdx.database import cascade as cascade_db

        # Add documents via cascade module
        doc_id = save_document("Test", "Content")
        cascade_db.update_cascade_stage(doc_id, "idea")

        # Use old function - should read from new table
