...
class TestArchiveCommand:
    ...
    @patch("emdx.commands.core.archive_document")
    @patch("emdx.commands.core.get_document")
    @patch("emdx.commands.core.db")
    def test_archive(self, mock_db, mock_get_doc, mock_archive):
        """Archive a document."""
        mock_db.ensure_schema = Mock()
        mock_get_doc.return_value = {
            "id": 1,
    ...
    @patch("emdx.commands.core.get_document")
    @patch("emdx.commands.core.db")
    def test_archive_not_found(self, mock_db, mock_get_doc):
        """Archive nonexistent document."""
        mock_db.ensure_schema = Mock()
        mock_get_doc.return_value = None

        result = runner.invoke(app, ["archive", "999"])
        assert result.exit_code != 0

    @patch("emdx.commands.core.get_document")
    @patch("emdx.commands.core.db")
    def test_archive_already_archived(self, mock_db, mock_get_doc):
        """Archive already archived document shows message."""
        mock_db.ensure_schema = Mock()
        mock_get_doc.return_value = {
            "id": 1,
        ...
        result = runner.invoke(app, ["archive", "1"])
        assert result.exit_code == 0
        assert "already archived" in _out(result)

    @patch("emdx.commands.core.archive_descendants")
    @patch("emdx.commands.core.archive_document")
    @patch("emdx.commands.core.get_document")
    @patch("emdx.commands.core.db")
    def test_archive_with_descendants(self, mock_db, mock_get_doc, mock_archive, mock_desc):
        """Archive with --descendants."""
        mock_db.ensure_schema = Mock()
        mock_get_doc.return_value = {
            "id": 1,
            "title": "Parent",
