...
def test_migration_creates_numeric_id_column(test_db):
    ...
    from emdx.database.migrations import migration_000_create_documents_table, migration_001_add_tags, migration_004_add_execution_pid
...
def test_get_recent_executions_with_numeric_ids(test_db):
    ...
    for i in range(5):
        exec_id = create_execution(
            doc_id=500 + i,
            doc_title=f"Recent Test {i}",
            log_file=f"/tmp/recent{i}.log"
        )
        ids.append(exec_id)
...
def test_migration_idempotency(test_db):
    ...
    from emdx.database.migrations import migration_000_create_documents_table, migration_001_add_tags, migration_003_add_document_relationships, migration_004_add_execution_pid
    migration_000_create_documents_table(conn)
    migration_001_add_tags(conn)
    migration_002_add_executions(conn)
