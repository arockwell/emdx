...
3. **Classify each gameplan**:
   - **Done** — All planned work has been merged → `emdx tag <id> done success && emdx untag <id> active`
   - **Partial** — Some work done, some remaining → keep `active`, note what's left
   - **Stale** — No progress and no longer relevant → `emdx untag <id> active`
   - **Blocked** — Can't proceed for a reason → `emdx tag <id> blocked && emdx untag <id> active`
...
5. **Create tasks for remaining work**:
   ```bash
   emdx task create "Title" --description "From gameplan #XXXX: ..."
   ```
